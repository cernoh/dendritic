...
| `paseo` | Paseo daemon and its bundled web UI as a Docker container, generated by [compose2nix](https://github.com/aksiksi/compose2nix) from `compose.yaml`; state lives on the 2TB ext4 disk, a child image adds the agent CLIs, and `tailscale serve` publishes the UI on the tailnet. NIXPC only |
| `posy-cursors` | Posy cursor themes |
| `programming` | Dev environment: git, direnv, tmux, zellij, gh, editors' companions |
| `retrosmart-cursor` | Retrosmart `mac-ish` cursor theme, recolored with the sepia palette (`outline` = `text`, `fill` = `base`); built from a pinned source tree and set as the pointer cursor on both hosts |
...
| `wayfinder-dashboard` | Wayfinder grill history + map switcher as a Deno Docker container, generated by [compose2nix](https://github.com/aksiksi/compose2nix) from `compose.yaml`; omp registers maps, history lives on the 2TB ext4 disk, and `tailscale serve` publishes the UI on the tailnet. NIXPC only |
