...
- **`home-manager` module is `moduleWithSystem`-free for `desktop` but this module itself is plain:** it reads `config.dendritic.userName` to wire `home-manager.users.<name>`. The user submodule also exports its own option tree as the internal `dendritic.nixdOptionTree`; `features/nvf` reads it back for nixd completion, because the imported feature modules appear in no other tree (`...users.type.getSubOptions []` carries the shared modules alone).
