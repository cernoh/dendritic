...
- **Feature table (current):** `act`, `agent-browser`, `brave`, `clipboard`, `computer-use`, `davinci`, `docker` (+ `mcp-containers.nix`), `dojjo`, `fish`, `gaming-tools`, `ghostty`, `lazygit`, `leetcode`, `mango`, `niri`, `nixpc-desktop`, `noctalia`, `noctalia-greeter`, `nushell`, `nvf`, `omp`, `opencode`, `posy-cursors`, `programming`, `scheme`, `steam`, `stremio-kai`, `usb-automount`, `wayland-base`, `widevine`.
- **`scheme` owns every color.** It exports `flake.scheme` (roles, base16, ANSI, noctalia/greeter/omp documents, wallpaper). A themed feature MUST read `self.scheme` instead of a local hex. Features that cannot read nix values keep a copy and name the file to sync.
