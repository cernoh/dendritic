...
description: "Prove an omp extension, custom tool, or interactive bridge in a live session over omp --mode rpc: the JSONL harness, waiting for idle instead of guessing, the essential-vs-discoverable tool trap, verifying a hidden skill's command, and the failure modes that cost real debug cycles"
---
