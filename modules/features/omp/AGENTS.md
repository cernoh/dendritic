...
- `home/agent/extensions/` — extension modules that omp discovers and loads in every session; `html-report.ts` registers the `render_html` tool
...
- **The report palette comes from `self.scheme`:** `home.activation.ompHtmlTheme` writes `~/.omp/agent/html-theme.json` from `self.scheme.html`, and `render_html` reads it for the desktop theme. The page keeps light and dark built in, so it still reads when that file is absent.
