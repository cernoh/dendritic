...
- **`parts.nix` also sets `debug = true`:** the flake exposes the flake-parts option trees (`debug.options`, `currentSystem.options`), which nixd reads for `flake.*` and per-system option completion (`features/AGENTS.md`). Removing it silently empties those completions.
