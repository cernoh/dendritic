# NIXPC Noctalia settings: translation of this host's exported shell settings.
#
# Plain data file. The `_` prefix keeps import-tree from auto-importing it (the
# same convention as modules/hosts/ASAHI/_noctalia-settings.nix);
# nixpcConfiguration.nix imports it into programs.noctalia.settings.
#
# Adaptations vs the export (everything else is verbatim):
# - plugins.enabled: the export lists only the plugins that come from a plugin
#   source. The cernoh/terminal plugin comes from the out-of-store symlink in
